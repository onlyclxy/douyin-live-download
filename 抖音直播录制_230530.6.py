...
version=230530.6  #请注意把下面这个地方和文件名改了就行啦
#pyinstaller -F 抖音直播录制_230530.6.py
#pyinstaller 抖音直播录制_230530.6.py
    ...
    try:  
        for _i in range(10):
            ...
            if len(res.text)<5 or res.text.find("繁忙")>=0:
                ...
                # print("端口网址为:"+url)
                # #写入txt源码.不用请去掉
                # with open("yuanma.txt", "w",encoding="utf8") as file:
                #     file.write(res.text)
                return res.text       
...
def newgeturl_usercount(url):
    ...
    try:  
        ...
        #获取直播间状态.2是直播.4是直播结束
        status=resjs["data"]["data"][0]["status"]["user_count_str"] #直播间人数
...
def startgo(line,countvariable=-1):
    ...
    while True:
        try:
            ...
            rid=ridcontent[0]
            if ridcontent[1]!="":
                print("运行新线程,传入地址"+ridcontent[0],ridcontent[1]+" 第"+str(countvariable)+"行")
            else:
                print("运行新线程,传入地址"+ridcontent[0]+" 第"+str(countvariable)+"行")
            ...
            while True:        
                try:
                    ...
                    if len(portInfo)==4:
                        ...
                        if portInfo[1]==2:
                            print(portInfo[0]+" 正在直播中 第"+str(countvariable)+"行")
                            ...
                            print("第"+str(countvariable) + "行 " + portInfo[0] + " 等待直播.. ")
                            startname=portInfo[0]
                        

                    else:
                        if len(startname)==0:
                            print('第'+ str(countvariable) +' 行网址内容获取失败,进行重试中...获取失败的地址是:'+ str(line))
                        else:
                            print('第'+ str(countvariable) +' 行网址内容获取失败,进行重试中...获取失败的地址是:'+ str(line)+" 主播为:"+str(startname))
                        warning_count+=1
...
while True:    
    ...
    try:
        ...
        config.read('config.ini', encoding='utf-8-sig')
        cookiesSet_temp=config.get('1', 'cookies')
        if cookiesSet_temp != "":
        ...
        for line in file:   
            ...
            if len(c)==0:
                ...
                if resline.find("https://live.douyin.com/")>-1 or resline.find("https://v.douyin.com/")>-1:
                    ...
                    #因为后面需要网址假主播名字,这里再补上
                    if len(SplitTest)>1:
                        lastdyurl= (resline,SplitTest[1],a)
                    else:
                        lastdyurl=(resline,"",a)
            ...
            for i in textNoRepeatUrl:
                ...
                if i[0] not in runingList:
                    if firstStart==False: #第一次 不会出现新增链接的字眼
                        print("新增链接: "+ str(i[0]) +" 在url配置的第"+str(i[2]) +"行")
