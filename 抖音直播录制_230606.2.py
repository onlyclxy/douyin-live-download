...
work = Queue()
#-------------------------------------
live_streaming=[] #正在直播的
live_all=[]
nolive_streaming=[] #未直播的
#------------配置变量
Monitoring=0
tsconvertmp4=""
delFilebeforeconversion=""
tsconvertm4a=""
delFilebeforeconversion=""
...
# onlybrowser=False   
# cookiesSet=""
headers = {'User-Agent': 'Mozilla/5.0 (Linux; U; Android 8.1.0; en-US; Nexus 6P Build/OPM7.181205.001) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/57.0.2987.108 UCBrowser/12.11.1.1197 Mobile Safari/537.36'}
...
def get_cookies():
    try:
        url = 'https://live.douyin.com/'  # 替换为你想要获取cookie的网址
        ...
        new_string = cookie.name + "=" +  cookie.value

        if len(str(new_string)) > 5:
            return new_string
        else:
            return ""
    except:
            ...
            try:
                ...
                try:
                    ...
                    if zhibo_id==zhibo_ids:
                        return([zhubo_name,2,hls_pull_url,rtmp_pull_url]) #因为上面出错到不了这里. 所以这里直接设置状态为2
                    else:
                        continue
                
                except:
                    return([zhubo_name,4,hls_pull_url,rtmp_pull_url]) 
                    pass
...
def displayinfo():
    ...
    while True:
        try:
            ...
            print("\r共监测"+str(len(live_all))+"个直播中", end=" | ")
            #以前的信息
            ...
            print("排队读取网址时间(秒): "+str(localdelaydefault), end=" | ")
            nowdate=time.strftime("%H:%M:%S", time.localtime())
            print(str(nowdate))
            if len(nolive_streaming)>0:
                print("未直播的共:"+str(len(nolive_streaming))+str(nolive_streaming))
...
def get_url_state(line):
    ...
    while True:
        try:
            ...
            rid=ridcontent[0]
            # if ridcontent[1]!="":
            #     print("运行新线程,传入地址"+str(ridcontent[0]),str(ridcontent[1])+" 序号"+str(countvariable))
            # else:
            #     print("运行新线程,传入地址"+str(ridcontent[0])+" 序号"+str(countvariable))
...
def down_live(portInfo,info):  
    global live_streaming,videopath,namelist,nolive_streaming
    countvariable=-1
    # portInfo =portInfo_group[0]
    # info =portInfo_group[1]
    recordfinish_2=""
    live_streaming.append(info)  

    if portInfo[0] in nolive_streaming:
        nolive_streaming.remove(portInfo[0]) #删除主播名字,这个是没有直播的人
    ...
    if len(portInfo)==4:
        ...
        if startname in allLive:
            ...
            namelist.append(str(rid)+"|"+str("#"+rid))
            exit(0)
        if portInfo[0]=="" and Runonce==False:                
            namelist.append(str(portInfo[2])+"|"+str(portInfo[2]+",主播: "+startname.strip()))  
            Runonce=True
    ...
    if info in live_streaming:
        live_streaming.remove(info)     # 删除 b 元素    
    nolive_streaming.append(portInfo[0]) #增加未录制主播名字


def stargo_single():
    global Monitoring,nolive_streaming
    i=0
    while(True):
        i=i+1
        if not work.empty():
            ...
            if info not in live_streaming:
                ...
                if len(portInfo)==4 and portInfo[1]==2:
                    ...
                    args = (portInfo,info)
                    print("检测到正在直播的 :"+str(portInfo[0]))
                    createVar['thread'+ str(Monitoring)] = threading.Thread(target=down_live,args=args)
                    createVar['thread'+ str(Monitoring)].start()
                elif len(portInfo)==4:
                    #未在直播的
                    if portInfo[0] not in nolive_streaming:
                        nolive_streaming.append(portInfo[0])
                    nolive_streaming=list(set(nolive_streaming)) #排重
...
def read_ini():
    ...
    while True:    
        ...
        try:
            config = configparser.ConfigParser()
            config.read('config.ini', encoding='utf-8-sig')
            line_count=config.getint('1', '线程数')
        except:
            config = configparser.ConfigParser()
            # -read读取ini文件
            config.read('config.ini', encoding='utf-8-sig')
            ...
            if '1' not in listx:# 如果分组type不存在则插入type分组
                ...
                config.remove_option('1', '线程数')# 删除type分组的stuno
                # config.remove_section('tpye')# 删除配置文件中type分组

            config.set('1', '线程数', '3')# 给type分组设置值
        ...
        try:
            ...
            config.set('1', '排队读取网址时间(秒)', '1')# 给type分组设置值
            ...
            if len(textNoRepeatUrl)>0:
                for i in textNoRepeatUrl:
                    ...
                    if i[0] not in runingList:
                        ...
                        if i[0] not in live_all:
                            live_all.append(i[0])
                            # work.put_nowait(i[0])
                            work.put(i[0], block=False, timeout=0) #放到最前面
...
if __name__ == '__main__':
    ...
    Number_of_Threads=line_count
    print("本次开启 "+str(Number_of_Threads)+" 个线程")
    for x in range(Number_of_Threads):
    #相当于创建了2个爬虫
        task = gevent.spawn(stargo_single)
